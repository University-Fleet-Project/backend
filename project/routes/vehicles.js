...
router.post('/check-availability',requireAuth,async(req,res)=>{const {vehicleId,startTime,endTime}=req.body;const passengers=req.body.passengers??req.body.requestedPassengers;const distanceKm=req.body.distanceKm??req.body.distance_km;const durationMinutes=req.body.durationMinutes??req.body.duration_minutes??req.body.duration;if(!vehicleId||!startTime)return fail(res,400,'VALIDATION_ERROR','vehicleId and startTime are required.');try{const result=await checkVehicleAvailability(pool,vehicleId,startTime,endTime,{requestedPassengers:passengers,distanceKm,durationMinutes});if(result.code==='INVALID_TIMEFRAME')return fail(res,400,'INVALID_TIMEFRAME',result.message);if(result.code==='VEHICLE_NOT_FOUND')return fail(res,404,'VEHICLE_NOT_FOUND',result.message);return ok(res,{available:result.available,isOperable:result.isOperable??false,serviceStatus:result.serviceStatus??null,conflicts:result.conflicts??[],...(result.code?{code:result.code}:{}),...(result.message?{message:result.message}:{}),...(result.totalCapacity!=null?{totalCapacity:result.totalCapacity}:{}),...(result.bookedPassengers!=null?{bookedPassengers:result.bookedPassengers}:{}),...(result.availableSeats!=null?{availableSeats:result.availableSeats}:{}),...(result.requestedPassengers!=null?{requestedPassengers:result.requestedPassengers}:{}),...(result.startTime?{startTime:result.startTime,start_time:result.startTime}:{}),...(result.endTime?{endTime:result.endTime,end_time:result.endTime}:{})});}catch(e){return fail(res,500,'AVAILABILITY_ERROR','Unable to check availability.');}});
