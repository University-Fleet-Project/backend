...
async function handleCreateUser(req,res){
  const {name,role,email,password='password',licenseNumber,license_number,status}=req.body;
  if(!name||!role||!email)return fail(res,400,'VALIDATION_ERROR','name, role and email are required.');
  const normRole=normalizeRole(role);
  const allowed=['requester','dispatcher','driver','fleet_admin','auditor'];
  if(!allowed.includes(normRole))return fail(res,400,'INVALID_ROLE','Invalid role.');

  const client=await pool.connect();
  try{
    await client.query('BEGIN');
    const existingCred=await client.query(`SELECT 1 FROM fleet_api_credentials WHERE lower(email)=lower($1)`,[email]);
    if(existingCred.rows[0]){
      await client.query('ROLLBACK');
      return fail(res,409,'CREATE_USER_ERROR','User or email already exists.');
    }
    const r=await client.query(`INSERT INTO users(name,role) VALUES($1,$2) RETURNING *`,[name,normRole]);
    const user=r.rows[0];
    const passHash=await bcrypt.hash(password,12);
    await client.query(`INSERT INTO fleet_api_credentials(user_id,email,password_hash) VALUES($1,$2,$3)`,[String(user.user_id),email,passHash]);

    let driverData=null;
    if(normRole==='driver'){
      const lic=licenseNumber||license_number||null;
      const drStatus=status||'available';
      const drRes=await client.query(`INSERT INTO drivers(user_id,status,license_number) VALUES($1,$2,$3) RETURNING *`,[String(user.user_id),drStatus,lic]);
      driverData=drRes.rows[0];
      await audit(req.user?.sub,'CREATE_DRIVER','driver',driverData.driver_id);
    }
    await client.query('COMMIT');
    await audit(req.user?.sub,'CREATE_USER','user',user.user_id);
    const responseData={...user,id:String(user.user_id),email,role:normRole};
    if(driverData){
      responseData.driver_id=driverData.driver_id;
      responseData.driver=driverData;
    }
    return ok(res,responseData,'User created',201);
  }catch(e){
    try{await client.query('ROLLBACK');}catch(_){}
    console.error(e);
    if(e.code==='23505') return fail(res,409,'CREATE_USER_ERROR','User or email already exists.');
    return fail(res,400,'CREATE_USER_ERROR','Unable to create user.');
  }finally{
    client.release();
  }
}
router.post('/',requireAuth,allowRoles('fleet_admin'),handleCreateUser);
...
router.post('/register',async(req,res)=>{const {name,email,password='password'}=req.body;const role='requester';if(!name||!email)return fail(res,400,'VALIDATION_ERROR','name and email are required.');try{const r=await pool.query(`INSERT INTO users(name,role) VALUES($1,$2) RETURNING *`,[name,role]);await ensureCredential(r.rows[0],email,password);return ok(res,{...r.rows[0],email,role},'User registered',201);}catch(e){return fail(res,400,'REGISTER_ERROR','Unable to register user.');}});
router.handleCreateUser=handleCreateUser;
